...
void add(int i, int x) {
    if ((space[x]!=-65536)&&(x>=0)&&(x<spacenumber))
...
void copyto(int i, int x) {
    if (current_block != -65536 && x<spacenumber && x>=0)
...
void copyfrom(int i, int x) {
    if (space[x] != -65536 && x<spacenumber && x>=0)
        current_block=space[x];
    else{
        cout << "Error on instruction " << (i + 1) << endl;
        error_flag = true;
...
void run() {
    ...
    while (i < instruction_count) {
        if (error_flag)
            break;
        if (instructions[i].name == "inbox"&&instructions[i].is_num_int) {
            inbox(i);
        } else if (instructions[i].name == "outbox"&&instructions[i].is_num_int) {
            outbox(i);
