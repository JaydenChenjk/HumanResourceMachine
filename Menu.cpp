...
{
    int count;
    flevel_in>> count;//可能出错！用全局变量接着读可能会出错，后续排查！//需要先输入有几个值！
    flevel_in.ignore();
    for (int i = 0; i < count; i++) 
    {
        string in;
        cin>>in;//存储的时候一行存多个数值！；可能在换行的时候出现困难
        inputs.push_back(in);
    }
    return count;
}

void Load_init_int(vector<int> &input)
{
...
void load_instructions_from_file(const string& file_path) {
    ...
    try {
        ...
        file.ignore();
        last_instructions.resize(instruction_count);
        for (int i = 0; i < instruction_count; i++) {
            string instruction;
            getline(cin, instruction);
            for(int j=0;j<instruction.length();j++) {
                if(instruction[j]==' ') {
                    last_instructions[i].number=instruction[j+1];
                    break;
                }
                last_instructions[i].name+=instruction[j];
            }
        }
    }
    catch (...) {
...
int initial(vector<int> &input,vector<int> &target,vector<string> &availables)//初始化函数，关卡数据存储格式：输入函数、目标、可用指令；
{
    ...
    cout<<"Please enter the number of the level you want to play."<<endl;
    cin>>levels;//endl
    try {
        ...
        else
        {
            ...
            else
            {
                ...
                load_instructions_from_file("C://Users//fyh_1//Desktop//programme//cxsjjc//Ming's robot//user_level"+levels);//按行存储上一次的指令
                flevel_in>>spnum;
                if(!flevel_in.eof())
...
int main()
{
    ...
    int spnum;
    do{
        spnum = initial(input,target,availables);//初始化;spnum:space number
    }while(!(spnum+1));
    spaces.resize(spnum);
