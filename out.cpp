...
void outbox(int i,int out_index, Sequence* rb_se,Scene* scene, Text* rb_text, Delay* delay) {
    if (current_block != INT_MIN) {
        ...
        int a = current_block;
        auto create_out = gcnew CallFunc([out_index, scene, a]() {
            out_brick[out_index + 1] = new Node();
        ...
        auto new_seq = gcnew CallFunc([out_index]() {
            if (out_index > 0)
            {
                auto seq = new Sequence;
        ...
        auto outbox_act = gcnew Spawn({changeText, create_out, new_seq});
...
void add(int i, int x, Sequence* rb_se, Text* rb_text, Delay* delay) {
...
void sub(int i, int x, Sequence* rb_se, Text* rb_text, Delay* delay) {
    if ((space[x] != INT_MIN) && (x >= 0) && (x < spacenumber)) {
        current_block -= space[x];
...
void copyto(int i, int x, Sequence* rb_se, Text* rb_text, Delay* delay) {
    if (current_block != INT_MIN && x < spacenumber && x >= 0) {
        ...
        auto moveTow = gcnew MoveTo(2.5f, Point(300 + 50 * x, 400));
        string s = std::to_string(current_block);
        auto changeText = gcnew CallFunc([rb_text, s]() {rb_text->setText(s); });
...
void copyfrom(int i, int x, Sequence* rb_se, Text* rb_text, Delay* delay){
...
bool run(Scene* scene) {
    ...
    if (level == 1) {
        ...
        for (int i = 0; i < instruction_count; i++) {
            ...
            display[i].setPos(1415, 150 + i * 30);
    ...
    {
        out_brick[i]->runAction(out_bricks_ani[1]->clone());
    }
    check();
    Game::start();
    rb_se->release();
