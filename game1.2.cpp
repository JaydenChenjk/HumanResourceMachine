...
void load_instructions_from_keyboard() {
    try {
        ...
        for (int i = 0; i < instruction_count; i++) {
            string instruction;
            string tmps;//临时存放所得指令的字符串，省去思考怎么初始化instructions[i].name的麻烦，但如果有时间可以尝试省去这个变量
...
int initial(/*vector<int> &input,vector<int> &target,vector<string> &availables*/)//初始化函数，关卡数据存储格式：输入函数、目标、可用指令；
{
    ...
    try {
        ...
        flevel_in.open("C:\\Users\\fyh_1\\Desktop\\programme\\cxsjjc\\Ming's robot\\level0.txt");
        flevel_in.close();
...
void inbox(int i) {
    if (indexx < input.size()) {
        current_block = input[indexx];
        indexx++;
    }
    else
...
bool run1() {
    for (int i = 0; i < instructions.size(); i++) {
        ...
        if (instructions[i].name == "inbox")
            ...
            cout << "Error on instruction " << (i + 1) << endl;//是不是可以直接break？
    }
